  ...
  return (
    <div style={{minHeight:'100vh',display:'flex',fontFamily:"'Segoe UI',sans-serif",background:'var(--bg)',transition:'background 0.3s'}}>
      <style>{`
        ...
        @keyframes glow2{0%,100%{box-shadow:0 0 20px rgba(34,197,94,0.4)}50%{box-shadow:0 0 50px rgba(34,197,94,0.7)}}
        ...
        @keyframes gg{0%,100%{border-color:rgba(34,197,94,0.3)}50%{border-color:rgba(34,197,94,0.6)}}
        ...
        .bmain2{width:100%;padding:14px;border:none;border-radius:11px;font-size:16px;font-weight:700;cursor:pointer;font-family:inherit;background:linear-gradient(135deg,#16a34a,#2563eb);color:white;transition:all 0.3s;animation:glow2 3s ease-in-out infinite}
        .bmain2:hover{transform:translateY(-2px);box-shadow:0 12px 32px rgba(34,197,94,0.5)}
        .bmain2:disabled{opacity:0.5;cursor:not-allowed;transform:none;animation:none}
        .brow{display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid var(--border)}
        .brow:last-child{border-bottom:none}
        .reg-left{display:flex;flex:1;position:relative;flex-direction:column;justify-content:center;padding:60px 64px;overflow:hidden}
        .reg-right{width:440px;flex-shrink:0;display:flex;flex-direction:column;justify-content:center;padding:0 44px;background:var(--card-bg);border-left:1px solid var(--border);backdrop-filter:blur(20px);transition:background 0.3s;animation:sr 0.7s ease both}
        @media(max-width:768px){
          .reg-left{display:none !important}
          .reg-right{width:100% !important;padding:40px 24px !important;border-left:none !important;justify-content:flex-start !important;padding-top:48px !important;animation:none !important}
          .mob-logo2{display:flex !important}
        }
        .mob-logo2{display:none;align-items:center;gap:10px;margin-bottom:28px}
      `}</style>
      ...
      <div className="reg-left">
        ...
        <div style={{position:'relative',zIndex:1,maxWidth:'500px',animation:'sl 0.7s ease both'}}>
          <div style={{display:'flex',alignItems:'center',gap:'14px',marginBottom:'40px'}}>
            <div style={{width:'52px',height:'52px',background:'linear-gradient(135deg,#16a34a,#2563eb)',borderRadius:'15px',display:'flex',alignItems:'center',justifyContent:'center',animation:'glow2 3s ease-in-out infinite',flexShrink:0}}>
      ...
      <div className="reg-right">
        {/* Mobile logo */}
        <div className="mob-logo2">
          <div style={{width:'38px',height:'38px',background:'linear-gradient(135deg,#16a34a,#2563eb)',borderRadius:'11px',display:'flex',alignItems:'center',justifyContent:'center'}}>
            <span style={{color:'white',fontSize:'18px',fontWeight:'900'}}>R</span>
          </div>
          <div style={{display:'flex',alignItems:'baseline'}}>
            {'ResumeAI'.split('').map((ch,i)=>(
              <span key={i} style={{fontSize:'22px',fontWeight:'900',color:i>=6?c.ai:'var(--text)'}}>{ch}</span>
            ))}
          </div>
        </div>

        <div style={{animation:'fu 0.5s ease 0.1s both',marginBottom:'22px'}}>
          <h2 style={{fontSize:'28px',fontWeight:'900',color:'var(--text)',marginBottom:'10px'}}>Create your account</h2>
          <div style={{display:'inline-flex',alignItems:'center',gap:'7px',background:'rgba(34,197,94,0.08)',border:'1px solid rgba(34,197,94,0.25)',borderRadius:'99px',padding:'5px 14px',animation:'gg 2.5s ease-in-out infinite'}}>
            <span style={{width:'7px',height:'7px',borderRadius:'50%',background:'#22c55e',boxShadow:'0 0 5px #22c55e',flexShrink:0}}/>
            <span style={{color:c.ai,fontSize:'13px',fontWeight:'700'}}>Free forever · 25 scans/day · No credit card</span>
          </div>
        </div>
        {error&&<div style={{background:'rgba(239,68,68,0.1)',border:'1px solid rgba(239,68,68,0.25)',color:dark?'#f87171':'#dc2626',borderRadius:'10px',padding:'12px 16px',fontSize:'14px',marginBottom:'16px'}}>{error}</div>}
        <form onSubmit={onSubmit}>
          {[{label:'Full name',name:'name',type:'text',ph:'Rahul Sharma'},{label:'Email',name:'email',type:'email',ph:'you@example.com'},{label:'Password',name:'password',type:'password',ph:'Min 6 characters'}].map((f,i)=>(
            <div key={f.name} style={{marginBottom:i<2?'16px':'24px',animation:`fu 0.5s ease ${0.2+i*0.08}s both`}}>
              <label style={{display:'block',fontSize:'12px',fontWeight:'700',color:'var(--text4)',marginBottom:'8px',textTransform:'uppercase',letterSpacing:'0.08em'}}>{f.label}</label>
          ...
          <div style={{animation:'fu 0.5s ease 0.44s both',marginBottom:'24px'}}>
            <button type="submit" disabled={loading} className="bmain2">{loading?'Creating account...':'Create free account →'}</button>
          </div>
        </form>
        <p style={{textAlign:'center',fontSize:'15px',color:'var(--text4)',margin:0,animation:'fu 0.5s ease 0.5s both'}}>
