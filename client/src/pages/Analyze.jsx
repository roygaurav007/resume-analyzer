...
const PixelKnight = ({ frame }) => {
  const palette = { 'H':'#7c8cbc','V':'#1a1a2e','A':'#5a6aa0','S':'#e8e8ff','G':'#f5c842','B':'#3a3a5c','W':'#ffffff','.':null }
  const px = 5
  const grid = [
    ['.','.','.','.',  'H','H','H','H','H','H','H','H','.','.','.','.'],
    ['.','.','.','.',  'H','V','V','V','V','V','V','H','.','.','.','.'],
  ...
  return (
    <svg width={16*px} height={grid.length*px} style={{imageRendering:'pixelated'}}>
      {grid.map((row,ri)=>row.map((cell,ci)=>{
        const color=palette[cell]; if(!color) return null
...
const KnightMascot = ({ uploaded }) => {
  ...
  useEffect(()=>{const t=setInterval(()=>setMsgIdx(i=>(i+1)%msgs.length),4000);return()=>clearInterval(t)},[msgs.length])
  const bobY=Math.sin(bounce*Math.PI/10)*3
  return (
    <div style={{display:'flex',flexDirection:'column',alignItems:'center',userSelect:'none'}}>
      <div style={{background:'var(--card-bg)',border:'2px solid var(--border2)',borderRadius:'12px',padding:'10px 14px',maxWidth:'190px',textAlign:'center',position:'relative',marginBottom:'10px',minHeight:'44px',display:'flex',alignItems:'center',justifyContent:'center'}}>
        <p style={{color:'var(--text)',fontSize:'11px',fontFamily:'"Courier New",monospace',fontWeight:'700',lineHeight:1.5,margin:0}}>
          <Typewriter key={msgIdx} text={msgs[msgIdx]} speed={50}/>
        </p>
    ...
    <div style={{minHeight:'100vh',background:'var(--bg)',padding:'32px 16px',transition:'background 0.3s'}}>
      <style>{`
        ...
        @media(max-width:640px){
          ...
          .knight-col{flex-direction:row !important;justify-content:center;padding-top:0 !important;padding:12px 0 !important;order:-1}
        }
      `}</style>
